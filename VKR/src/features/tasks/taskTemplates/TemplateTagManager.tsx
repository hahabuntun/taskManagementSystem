...
import {
  useGetAllTags,
  useAddExistingTemplateTag,
  useAddNewTemplateTag,
  useDeleteTemplateTag,
  useUpdateTemplateTags,
} from "../../../api/hooks/tags";
...
interface TaskTemplateTagManagerProps {
  templateId: number;
}

export const TaskTemplateTagManager = ({
  templateId,
}: TaskTemplateTagManagerProps) => {
  ...
  const { data: template, isLoading: templateLoading } =
    useGetTaskTemplate(templateId);
  const { data: existingTags = [], isLoading: tagsLoading } = useGetAllTags(
    TagOwnerEnum.TASK_TEMPLATE
  );
  const addExistingTagMutation = useAddExistingTemplateTag();
  const addNewTagMutation = useAddNewTemplateTag();
  const deleteTagMutation = useDeleteTemplateTag();
  const updateTagsMutation = useUpdateTemplateTags();

  useEffect(() => {
    if (template?.tags) {
      setSelectedExistingTagIds(template.tags.map((tag) => tag.id));
      setNewTags([]);
    }
  }, [template]);
  ...
  const handleAddNewTag = async () => {
    if (newTagName.trim() && !isTagNameExists(newTagName.trim())) {
      ...
      try {
        const result = await addNewTagMutation.mutateAsync({ templateId, tag });
        if (result.id) {
          setNewTags([
            ...newTags,
  ...
  const handleRemoveExistingTag = async (tagId: number) => {
    try {
      const result = await deleteTagMutation.mutateAsync({ templateId, tagId });
      if (result.success) {
        setSelectedExistingTagIds(
  ...
  const handleRemoveNewTag = async (tagName: string) => {
    ...
    try {
      const result = await updateTagsMutation.mutateAsync({ templateId, tags });
      if (result.success) {
  ...
  if (templateLoading || tagsLoading) {
