...
export const ProjectTagManager = ({ projectId }: ProjectTagManagerProps) => {
  ...
  const { data: existingTags = [], isLoading: tagsLoading } = useGetAllTags(
    TagOwnerEnum.PROJECT
  );
  const addExistingTagMutation = useAddExistingProjectTag();
  const addNewTagMutation = useAddNewProjectTag();
  const deleteTagMutation = useDeleteProjectTag();
  const updateTagsMutation = useUpdateProjectTags();

  useEffect(() => {
    if (project?.tags) {
      setSelectedExistingTagIds(project.tags.map((tag) => tag.id));
      setNewTags([]);
    }
  }, [project]);
  ...
  const handleAddNewTag = async () => {
    if (newTagName.trim() && !isTagNameExists(newTagName.trim())) {
      ...
      try {
        const result = await addNewTagMutation.mutateAsync({ projectId, tag });
        if (result.id) {
          setNewTags([
            ...newTags,
  ...
  const handleRemoveExistingTag = async (tagId: number) => {
    try {
      const result = await deleteTagMutation.mutateAsync({ projectId, tagId });
      if (result.success) {
        setSelectedExistingTagIds(
  ...
  const handleRemoveNewTag = async (tagName: string) => {
    ...
    try {
      const result = await updateTagsMutation.mutateAsync({ projectId, tags });
      if (result.success) {
